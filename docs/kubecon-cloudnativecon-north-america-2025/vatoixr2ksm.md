...
Their architecture spans multiple cloud regions, organizing clusters into service, batch, and through (preemptible) categories to optimize for different workload patterns and resource utilization.
...
Capacity management is handled through a tiered system—reserved, elastic, and through—with custom autoscaling, disruption budgets, and node defragmentation processes to balance cost, launch latency, and disruption tolerance.
...
|   5 | Clusters are categorized as service (long-running), batch (short-lived), and through (temporary, preemptible).                                                                                                      |
|   6 | No unified entry point across regions; users specify region for workload placement.                                                                                                                                 |
|   7 | Cross-cluster through mechanism utilizes spare capacity from service clusters.                                                                                                                                      |
...
|  15 | Capacity management balances cost efficiency, launch latency, and disruption tolerance via three priority tiers: reserved (pre-scaled), elastic (scales as needed), and through (preemptible, uses spare capacity). |
|  16 | Autoscaling uses a two-stage node scaling down process, allowing through pods on scaling-down nodes and reversible taints.                                                                                          |
