...
To address these, they developed a system with two microservices: a drain coordinator that intelligently schedules and ranks nodes for maintenance, and a lifecycle orchestrator that manages state transitions, enforces SLAs, and handles notifications, all coordinated via a custom resource definition (CRD).
...
|   4 | Transitioned from manual to fully automated, seamless lifecycle management.                                                   |
...
|  15 | Lifecycle orchestrator (node-level): manages state transitions, enforces SLAs, sends notifications.                           |
|  16 | Communication between controllers via a maintenance CRD that mirrors node state.                                              |
|  17 | Extended maintenance lifecycle with clear states: scheduled, cordoned, drained, maintenance, complete, incomplete.            |
...
|  27 | Treat maintenance as a first-class lifecycle.                                                                                 |
